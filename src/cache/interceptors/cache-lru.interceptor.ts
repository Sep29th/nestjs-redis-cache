...
@Injectable()
export class CacheLruInterceptor implements NestInterceptor {
  ...
  async intercept(
    ...
    if (cachedResult) {
      ...
      this.cacheService.zAdd(sortedSetKeyName, [
        { score: new Date().getTime(), value: key },
      ]);
